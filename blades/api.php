...
Route::get('/commoditiesTypes', function (Request $request) {
    ...
    return view('commoditiesTypes', ['commodityTypes' => $tmp]);
});
...
Route::get('/conflicts', function (Request $request) {
    ...
    // Walidacja danych
    if ($request->has('start_date') && $request->has('start_date_2nd') && CompareDates::isGreater($request->get('start_date'), $request->get('start_date_2nd'))) {
        $errorMessage .= "start_date_2nd must be greater or equal than the start_date\n";
    }

    if ($request->has('start_date_2nd') && $request->has('end_date') && CompareDates::isGreater($request->get('start_date_2nd'), $request->get('end_date'))) {
        $errorMessage .= "end_date must be greater or equal than the start_date_2nd\n";
    }

    if ($request->has('end_date') && $request->has('start_date') && CompareDates::isGreater($request->get('start_date'), $request->get('end_date'))) {
        $errorMessage .= "end_date must be greater or equal than the start_date\n";
    }

    // Sprawdzenie regionów
    if ($request->has('region')) {
        $regions = explode(',', $request->input('region'));
        $tmp = getConflictsRegions();
        foreach ($regions as $region) {
            if (!in_array($region, $tmp)) {
                $errorMessage .= "Given region(s) is invalid\n";
                break;
            }
        }
    }
    ...
    if ($request->has('location')) {
        ...
        $tmp = getConflictsLocations();
        if (!in_array($location, $tmp)) {
            $errorMessage .= "Given location is invalid\n";
        }
    }

    // Jeżeli wystąpił błąd, rzucenie wyjątku
    if ($errorMessage !== "") {
        throw new Exception($errorMessage);
    }

    // Pobieranie konfliktów
    $query = Conflict::query();
    ...
    $conflicts = $query->get(); // Pobieranie wyników z bazy danych

    // Zwrócenie widoku z danymi
    return view('conflicts', compact('conflicts')); 
});
